...
function comparar($ultimo, $anterior)
{
    ...
    foreach ($acta1m as $eleccion => $acta1) {
        if ($acta1) {
...
$acta = cachedCargarActa('actas/acta.2019.10.25.21.09.30.xlsx');
$a = 'Presidente y Vicepresidente';
$mesas = array_keys($acta[$a]);

//$ttl = 10;
foreach ($mesas as $mesa) {
    if ($acta[$a][$mesa][0]!='Bolivia') continue;
    if ($acta[$a][$mesa][2]!='Beni') continue;

    $url = 'https://trep.oep.org.bo/resul/imgActa/' . $mesa . '1.jpg';
    $exif = exif_read_data($url);
    $date = $exif['GPSDateStamp'] ?? $exif['DateTimeOriginal'] ?? $exif['DateTimeDigitized'] ?? '';
    if ($date) echo "$mesa => $date\n";
    //print_r($exif);
    //$ttl--;
    //if (!$ttl) break;
}
